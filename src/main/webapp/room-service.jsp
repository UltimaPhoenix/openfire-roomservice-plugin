<%@ page import="java.util.*,
                 ...
                 org.jivesoftware.openfire.plugin.rooomservice.RoomServicePlugin"
    errorPage="error.jsp"
%>
...
<%  // Get parameters
    ...
    RoomServicePlugin plugin = (RoomServicePlugin) XMPPServer.getInstance().getPluginManager().getPlugin("roomservice");
    ...
    if (save) {
        if (errors.size() == 0) {
            ...
            response.sendRedirect("room-service.jsp?success=true");
            return;
        }
    }
...
<html>
    <head>
        <title>Room Service Properties</title>
        <meta name="pageID" content="room-service"/>
    </head>
    <body>


<p>
Use the form below to enable or disable the Room Service and configure the secret key.
By default the Room Service plugin is <strong>disabled</strong>, which means that
...
<%  if (success) { %>
    ...
    <tbody>
        ...
        <td class="jive-icon-label">
            Room service properties edited successfully.
        </td></tr>
    </tbody>
    </table>
    </div><br>
...
<form action="room-service.jsp?save" method="post">

<fieldset>
    <legend>Room Service</legend>
    <div>
    <p>
    The addition, deletion of room is not normally available outside of the admin console.
    ...
    <ul>
        ...
        <%= ((enabled) ? "checked" : "") %>>
        <label for="rb01"><b>Enabled</b> - Room service requests will be processed.</label>
        <br>
        <input type="radio" name="enabled" value="false" id="rb02"
         <%= ((!enabled) ? "checked" : "") %>>
        <label for="rb02"><b>Disabled</b> - Room service requests will be ignored.</label>
        <br><br>
